...
module Day08 =
    ...
    let nopP =
        pstring "nop " >>. pint64 |>> (int >> Nop)

    let incrP =
        pstring "acc " >>. pint64 |>> (int >> Incr)

    let jumpP =
        pstring "jmp " >>. pint64 |>> (int >> Jump)
    ...
    let interpret state =
        ...
        | Some (Nop _), false ->
            let newState =
                { state with
                      Line = state.Line + 1
                      Executed = Set.add state.Line state.Executed }
    ...
    module Part1 =
        let answer =
            lazy (program |> execute |> Seq.last |> (fun s -> s.Acc))
