...
module Day05 =
    ...
    let interpretDirs rows cols =
        { Row =
              List.ofArray rows
              |> List.mapi (fun i x -> x * (pown 2 (6 - i)))
              |> List.sum
          Col =
              List.ofArray cols
              |> List.mapi (fun i x -> x * (pown 2 (2 - i)))
              |> List.sum }
    ...
    let boardingPassP =
        pipe2 (parray 7 frontBackP) (parray 3 leftRightP) interpretDirs
        .>> newline
    ...
    let boardingPasses =
        Data.readFile ("day05") |> parseBoardingPasses

    module Part1 =
        let answer =
            lazy (boardingPasses |> List.map seatID |> List.max)
