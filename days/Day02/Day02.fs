...
module Day02 =
    ...
    let policyP =
        pipe3 minP maxP reqP (fun min max req -> { Min = min; Max = max; Letter = req })
    ...
    module Part1 =
        let auditMethod (policy: Policy) (pw: string) =
            let counts =
                pw |> Seq.toList |> List.countBy id |> Map.ofList
        ...
        let answer =
            lazy (passwords |> auditWith auditMethod |> countTrue)

    module Part2 =
        let xor a b = (a || b) && not (a && b)

        let newAuditMethod (policy: Policy) (pw: string) =
            policy.Min <= pw.Length
            && policy.Max <= pw.Length
            && policy.Min >= 1
            && policy.Max >= 1
            && xor (pw.[policy.Min - 1] = policy.Letter) (pw.[policy.Max - 1] = policy.Letter)

        let answer =
            lazy (passwords |> auditWith newAuditMethod |> countTrue)
