...
module Day07 =
    ...
    let colorP =
        charsTillString " bags contain " true 255

    let bagSpecP =
        pipe2 (pint64 .>> many1Chars (pchar ' ') |>> int) (charsTillString " bag" true 255) makeBagSpec
        ...
        let rec canContain env target key =
            ...
            | Some specs ->
                ...
                |> List.exists
                    (fun spec ->
                        spec.Color = target
                        || canContain env target spec.Color)

        let answer =
            lazy
                (rules
                 |> Map.filter (fun k _v -> canContain rules "shiny gold" k)
                 |> Map.count)

    module Part2 =
        let rec countContents env =
            function
            | None -> 0
            | Some specs ->
                List.fold
                    (fun acc x ->
                        x.Count
                        * (1 + countContents env (Map.find x.Color env))
                        + acc)
                    0
                    specs

        let answer =
            lazy (countContents rules (Map.find "shiny gold" rules))
